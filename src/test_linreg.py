...
def train_and_evaluate_linreg(X, y, model_name="Model"):
    ...
    return train_mae, test_mae, model, X_train.columns
...
results = {
    "Listings Only": train_and_evaluate_linreg(X_listings, y, "Listings Only"),
    "With Reviews": train_and_evaluate_linreg(X_with_reviews, y, "With Reviews"),
    "With Calendar": train_and_evaluate_linreg(X_with_calendar, y, "With Calendar"),
    "All Features": train_and_evaluate_linreg(X_all, y, "All Features")
}
...
plt.title('Linear Regression MAE by Feature Set')
plt.legend()
plt.tight_layout()
plt.show()
...
_, _, best_model, best_features = results[best_model_name]
coefs = best_model.coef_
importance_pairs = list(zip(best_features, np.abs(coefs)))
...
plt.title(f'Top 20 Most Important Features ({best_model_name}) - Linear Regression')
plt.tight_layout()
plt.show() 